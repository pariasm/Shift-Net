...
import time
import argparse
from basicsr.models.archs.gshift_denoise1 import GShiftNet
...
#                   noise = torch.empty_like(in_tensor).normal_(mean=0, std=std1) #.cuda().half()
                    ...
                    output2 = self.net(in_tensor[:,:,:,0:H//2+pad_h,W//2-pad:], std_map.expand(B, N, 1, H//2+pad_h, W//2+pad))
                    output3 = self.net(in_tensor[:,:,:,H//2-pad_h:,0:W//2+pad], std_map.expand(B, N, 1, H//2+pad_h, W//2+pad))
                    output4 = self.net(in_tensor[:,:,:,H//2-pad_h:,W//2-pad:], std_map.expand(B, N, 1, H//2+pad_h, W//2+pad))
...
if __name__ == '__main__':
    ...
    if args.default_data == 'DAVIS':
        ...
        args.result_path = 'infer_results/DAVIS/'
        ...
        args.model_path = 'pretrained_models/net_denoise.pth'
        args.result_path = 'infer_results/Set8/'
        if not os.path.exists(args.result_path):
            os.mkdir(args.result_path)
        args.result_path = args.result_path + "sigma%d"%(args.sigma)
        if not os.path.exists(args.result_path):
